...
def render():
    ...
    global alphaAng, betaAng, gammaAng
    ...
    # ZYX Euler angles
    M = np.identity(4)
    Rz2 = np.array([[np.cos(alphaAng), -np.sin(alphaAng), 0],
                   [np.sin(alphaAng), np.cos(alphaAng), 0],
                   [0,0,1]])
    Rx = np.array([[1,0,0],
                   [0, np.cos(betaAng), -np.sin(betaAng)],
                   [0, np.sin(betaAng), np.cos(betaAng)]])
    Rz1 = np.array([[np.cos(gammaAng), -np.sin(gammaAng), 0],
                   [np.sin(gammaAng), np.cos(gammaAng), 0],
                   [0,0,1]])
    M[:3,:3] = Rz2 @ Rx @ Rz1
    glMultMatrixf(M.T)
...
def key_callback(window, key, scancode, action, mods):
    ...
    if action==glfw.PRESS or action==glfw.REPEAT:
        if key==glfw.KEY_A:
            alphaAng += np.radians(10)
        elif key==glfw.KEY_Z:
            alphaAng -= np.radians(10)
        elif key==glfw.KEY_S:
            betaAng += np.radians(10)
        elif key==glfw.KEY_X:
            betaAng -= np.radians(10)
        elif key==glfw.KEY_D:
            gammaAng += np.radians(10)
        elif key==glfw.KEY_C:
            gammaAng -= np.radians(10)
        elif key == glfw.KEY_V:
            alphaAng = 0.
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2017029452-8-1', None,None)
    if not window:
        glfw.terminate()
        return
